		...
		<div style="text-align: left; margin: 50px 40px 20px 40px">
			...
			<form action="insertHelper" method="post">
				...
				<script>
					...
					// 	var helper_email = $("#email").val();
					// })
					// $.ajax({
					// 	type: "POST",
					// 	url: "/checkDuplicatedEmailWhenInsertHelper",
					// 	data: JSON.stringify({helper_email : helper_email}),
					// 	contentType: "application/json",
					// 	success: function (result) {
					// 		if(result === "Not duplicatedEmail") {
					...
					$("#authBtn").click(function(e) {
						...
						$.ajax({
							...
							success: function(response) {
								var responseObject = JSON.parse(response); // 응답을 JSON 객체로 변환
								console.log(responseObject.message);
								if(responseObject.message === "existEmail") {
									alert("이미 있는 이메일 주소입니다.");
									$('#email').focus();
								} else if(responseObject.authCode) {
									// 인증 코드가 정상적으로 전송된 경우
									alert("입력하신 주소로 인증 코드가 전송됐습니다.");
									console.log("인증 코드: " + responseObject.authCode);
									$("#authNum").focus();
								} else {
