...
import com.example.sanpa_com.biz.mother.impl.MotherDAO_mybatis;
...
public class SanpaController {
    ...
    public @ResponseBody String mailConfirm(@RequestParam String email) throws MessagingException, UnsupportedEncodingException, NumberFormatException {
        int isDuplicated = sanpaDAO.checkDuplicationEmail(email); // 이메일 주소만을 인자로 받는 메소드로 변경
        ...
        if(isDuplicated == 0){
            ...
            response.put("message", "existEmail");
        }
